...
RAW_SW_COLS = ["Speed", "Density", "Bt", "Bz"]
RAW_CORE_COLS = ["Date", "X", "Speed", "Density", "Bt", "Bz"]
MODEL_KEEP_COLS = [
    "Date",
    "X",
    "X_pert",
...
def _model_keep_cols(df: pd.DataFrame) -> list[str]:
    cols = MODEL_KEEP_COLS.copy()
    if "Kp" in df.columns:
        cols.append("Kp")
    return cols


def _raw_keep_cols(df: pd.DataFrame) -> list[str]:
    cols = RAW_CORE_COLS.copy()
    if "Kp" in df.columns:
        cols.append("Kp")
    return cols


def _ensure_datetime(df: pd.DataFrame, col: str = "Date", utc: bool | None = None) -> pd.DataFrame:
    df = df.copy()
    df[col] = pd.to_datetime(df[col], utc=utc, errors="coerce")
    return df.dropna(subset=[col])


def _strict_hourly_grid(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
...
def _to_float32_except_date(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
...
class ValidationGuard(Callback):
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        if self.best_val is None:
            self.best_val = val
            print(f"[Epoch {epoch:03d}] ValidationGuard: initialisation (val_loss={val:.6f})")
            return
        ...
        if val < self.best_val - self.min_delta:
            ...
            self.bad_epochs = 0
            print(
                f"[Epoch {epoch:03d}] ✅ amélioration val_loss={val:.6f} "
            ...
            self.bad_epochs += 1
            print(
                f"[Epoch {epoch:03d}] ⚠️ dégradation val_loss={val:.6f} "
            ...
            print(f"[Epoch {epoch:03d}] ➖ quasi stable val_loss={val:.6f} (best={self.best_val:.6f})")
...
class EpochStatsPrinter(Callback):
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        print(
            f"[Epoch {epoch:03d}] "
            f"train_loss={_fmt(metrics.get('train_loss'))} | "
            f"val_loss={_fmt(metrics.get('val_loss'))}"
        )
...
def propagate_l1_to_earth(df_sw: pd.DataFrame) -> pd.DataFrame:
    ...
    df_sw = df_sw.replace([np.inf, -np.inf], np.nan).dropna(subset=["prop_delay_h"])
    df_sw["Date"] = df_sw["Date"] + pd.to_timedelta(df_sw["prop_delay_h"], unit="h")
    df_sw = df_sw[["Date", "Speed", "Density", "Bt", "Bz"]].copy()
    return (
...
def load_and_prepare(data_dir: Path = DATA_DIR, use_cache: bool = True) -> pd.DataFrame:
    expected_cols = set(_model_keep_cols(pd.DataFrame(columns=["Kp"])))
    ...
    sw = pd.read_csv(data_dir / "solarwinds-ace-compiled" / "smooth-solarwinds.csv", sep=";")
    sw["Date"] = pd.to_datetime(sw["Date"], format="%Y%m%d%H%M%S")

    print("Loading Kiruna magnetometer (smooth.csv) …")
    kiruna = pd.read_csv(data_dir / "mag-kiruna-compiled" / "smooth.csv", sep=";")
    ...
    sw = propagate_l1_to_earth(sw).set_index("Date")

    df = kiruna.join(sw, how="inner").join(kp, how="left")
    df = df.interpolate(method="linear")
    df = df.dropna(subset=["X", "Speed", "Density", "Bt", "Bz"])
    df = _add_engineered_features(df.reset_index())
    df = _to_float32_except_date(df[_model_keep_cols(df)].copy())
...
def build_timeseries(df: pd.DataFrame):
    ...
    target = TimeSeries.from_dataframe(df, time_col="Date", value_cols=[TARGET_COL], fill_missing_dates=True, freq="1h").astype(np.float32)
    past_cov = TimeSeries.from_dataframe(df, time_col="Date", value_cols=PAST_COV_COLS, fill_missing_dates=True, freq="1h").astype(np.float32)
    future_cov = TimeSeries.from_dataframe(df, time_col="Date", value_cols=FUTURE_COV_COLS, fill_missing_dates=True, freq="1h").astype(np.float32)
        ...
        pl_trainer_kwargs={
            **accelerator_cfg,
            "callbacks": [
                ...
                EpochStatsPrinter(),
                ValidationGuard(degrade_patience=DEGRADE_PATIENCE, min_delta=DEGRADE_MIN_DELTA),
            ],
            "logger": CSVLogger(save_dir=str(MODEL_DIR / "logs"), name="aurora_tft"),
...
def load_trained_model() -> TFTModel:
    return TFTModel.load_from_checkpoint("aurora_tft", work_dir=str(MODEL_DIR), best=True, weights_only=False)


def _save_scalers(scaler_target, scaler_past, scaler_future):
    MODEL_DIR.mkdir(parents=True, exist_ok=True)
    with open(MODEL_DIR / "scalers.pkl", "wb") as f:
        pickle.dump({"target": scaler_target, "past_cov": scaler_past, "future_cov": scaler_future}, f)
...
def fetch_dscovr_realtime_2h() -> pd.DataFrame:
    plasma = _json_rows_to_frame(_http_get_json(DSCOVR_PLASMA_RT_URL)).rename(columns={"speed": "Speed", "density": "Density"})
    mag = _json_rows_to_frame(_http_get_json(DSCOVR_MAG_RT_URL)).rename(columns={"bt": "Bt", "bz_gsm": "Bz", "bz": "Bz"})

    plasma = plasma[[c for c in ["Date", "Speed", "Density"] if c in plasma.columns]].copy()
    mag = mag[[c for c in ["Date", "Bt", "Bz"] if c in mag.columns]].copy()
    df = pd.merge(plasma, mag, on="Date", how="outer").sort_values("Date")

    missing = [c for c in RAW_SW_COLS if c not in df.columns]
    if missing:
...
def load_local_dscovr_json_file(path: str | Path, file_type: str) -> pd.DataFrame:
    ...
    if file_type == "plasma":
        df = df.rename(columns={"speed": "Speed", "density": "Density"})
        needed = ["Date", "Speed", "Density"]
    elif file_type == "mag":
        df = df.rename(columns={"bt": "Bt", "bz_gsm": "Bz", "bz": "Bz"})
        needed = ["Date", "Bt", "Bz"]
    else:
    ...
    if missing:
        raise ValueError(f"Missing required {file_type} columns in {path.name}: {missing}")
    return df[needed].copy()


def _hourly_mean(df: pd.DataFrame) -> pd.DataFrame:
    return df.set_index("Date").resample("1h").mean().dropna().reset_index()


def _merge_raw_context(hist_raw: pd.DataFrame, future_raw: pd.DataFrame) -> pd.DataFrame:
    merged = pd.concat([hist_raw, future_raw], ignore_index=True).sort_values("Date")
    merged = merged.drop_duplicates(subset=["Date"], keep="last").reset_index(drop=True)
    merged = _strict_hourly_grid(merged)

    for col in ["X", *RAW_SW_COLS]:
        merged[col] = pd.to_numeric(merged[col], errors="coerce")
    merged[["X", *RAW_SW_COLS]] = merged[["X", *RAW_SW_COLS]].interpolate(method="linear", limit_direction="both")

    if "Kp" in merged.columns:
        merged["Kp"] = pd.to_numeric(merged["Kp"], errors="coerce")

    sw_earth = propagate_l1_to_earth(merged[["Date", *RAW_SW_COLS]].copy())
    merged = merged.drop(columns=RAW_SW_COLS)
    merged = pd.merge(merged, sw_earth, on="Date", how="left").sort_values("Date")
    merged[RAW_SW_COLS] = merged[RAW_SW_COLS].interpolate(method="linear", limit_direction="both")
    merged = _add_engineered_features(merged)
    merged = merged[_model_keep_cols(merged)].copy()
    return _to_float32_except_date(merged)
...
def load_recent_realtime_context(base_df: pd.DataFrame) -> pd.DataFrame:
    ...
    kiruna_h = _hourly_mean(kiruna_rt)
    dscovr_h = _hourly_mean(dscovr_rt)
    ...
    hist_raw = df[df["Date"] < min_live_date].copy()[_raw_keep_cols(df)]

    live_h["Kp"] = np.nan
    live_h = live_h[[c for c in ["Date", "X", *RAW_SW_COLS, "Kp"] if c in live_h.columns]]
    live_h["Date"] = pd.to_datetime(live_h["Date"], utc=True, errors="coerce").dt.tz_convert(None)

    merged = _merge_raw_context(hist_raw.assign(Date=pd.to_datetime(hist_raw["Date"], utc=True, errors="coerce").dt.tz_convert(None)), live_h)
    return merged.dropna(subset=["X", "Speed", "Density", "Bt", "Bz", "X_pert"]).reset_index(drop=True)


def load_dscovr_file_scenario_context(base_df: pd.DataFrame, plasma_path: str | Path, mag_path: str | Path) -> pd.DataFrame:
    df = base_df.copy()
    df["Date"] = pd.to_datetime(df["Date"], errors="coerce")
    df = df.dropna(subset=["Date"]).sort_values("Date").reset_index(drop=True)
    ...
    plasma_h = _hourly_mean(plasma)
    mag_h = _hourly_mean(mag)
    ...
    scenario_h["X"] = float(df["X"].iloc[-1])
    scenario_h["Kp"] = np.nan

    hist_raw = df[_raw_keep_cols(df)].copy()
    scen_raw = scenario_h[[c for c in ["Date", "X", *RAW_SW_COLS, "Kp"] if c in scenario_h.columns]].copy()
    merged = _merge_raw_context(hist_raw, scen_raw)
...
def _prepare_future_covariates(future_cov: TimeSeries, horizon: int, scaler_future: Scaler) -> TimeSeries:
    start_time = future_cov.start_time()
    end_time = future_cov.end_time() + pd.Timedelta(hours=horizon)
    ...
    future_cov_full = TimeSeries.from_times_and_values(full_times, full_future_vals, columns=FUTURE_COV_COLS)
    return scaler_future.transform(future_cov_full).astype(np.float32)


def _forecast_core(df: pd.DataFrame, n_hours: int, current_speed: float | None = None) -> pd.DataFrame:
    ...
    future_cov_sc_full = _prepare_future_covariates(future_cov, required_horizon, scaler_future)
    ...
    baseline_last = float(df["X"].rolling(ROLLING_BASELINE_HOURS, min_periods=1).mean().iloc[-1])

    effective_speed = current_speed if current_speed is not None else float(recent_past["Speed"].iloc[-1])
    ...
    if effective_speed > 0:
        ...
        print(
            ...
            f"le vent solaire met environ {tt_min:.0f} minutes ({tt_h:.2f} h) pour se propager de L1 à la Terre."
        )
...
def forecast(n_hours: int = 5, current_speed: float | None = None, use_realtime: bool = False):
    ...
    if use_realtime:
        try:
            ...
            print(f"Warning: unable to inject real-time data, fallback to local history only. Reason: {type(exc).__name__}: {exc}")

    results = _forecast_core(df, n_hours=n_hours, current_speed=current_speed)
    print("\n=== PRÉVISIONS ===")
    print(results.to_string(index=False))
    return results


def forecast_from_dscovr_files(plasma_path: str | Path, mag_path: str | Path, n_hours: int = 10, label: str | None = None):
    if n_hours <= 0:
        raise ValueError("--hours must be a positive integer")

    df = load_and_prepare(use_cache=True)
    df = load_dscovr_file_scenario_context(base_df=df, plasma_path=plasma_path, mag_path=mag_path)
    results = _forecast_core(df, n_hours=n_hours, current_speed=None)
...
def forecast_requested_combinations(mag_path: str | Path, mag_storm_path: str | Path, plasma_storm_path: str | Path, n_hours: int = 10):
...
def train_model(retrain: bool = False):
    ...
    print(
        f"Training samples: {len(ts_train)}, Validation samples: {len(ts_val)}, Test samples: {len(ts_test)} | "
...
def validate():
    ...
    df_eval = df_with_kp.set_index("Date").loc[bt.start_time(): bt.end_time()].copy()
    ...
    x_true, x_pred, df_eval = x_true[:n], x_pred[:n], df_eval.iloc[:n]

    mae_x = mean_absolute_error(x_true, x_pred)
    rmse_x = np.sqrt(mean_squared_error(x_true, x_pred))
    r_x = float("nan") if len(x_pred) < 2 or np.std(x_pred) == 0 or np.std(x_true) == 0 else float(np.corrcoef(x_pred, x_true)[0, 1])
    ...
    r_kp = float("nan") if len(kp_est) < 2 or np.std(kp_est) == 0 or np.std(kp_true) == 0 else float(np.corrcoef(kp_est, kp_true)[0, 1])
...
def main():
    ...
    p_fc.add_argument("--realtime", action="store_true", help="Inject latest Kiruna + DSCOVR real-time data before forecasting")
    ...
    if args.command == "train":
        ...
        forecast(n_hours=args.hours, current_speed=args.speed, use_realtime=args.realtime)
    elif args.command == "forecast-files":
        forecast_from_dscovr_files(plasma_path=args.plasma, mag_path=args.mag, n_hours=args.hours, label=args.label)
